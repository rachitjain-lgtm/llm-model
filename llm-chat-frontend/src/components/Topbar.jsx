...
export default function Topbar() {
  ...
  if (!activeChat) {
    return (
      <div className="h-16 border-b border-[#E7E7E7] dark:border-[#23272A] bg-white dark:bg-[#16191B] flex items-center px-6 transition-colors duration-200">
        <button onClick={() => dispatch(toggleSidebar())} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-[#23272A] mr-2 text-[#737373] dark:text-[#94A3B8] cursor-pointer" title={sidebarOpen ? "Hide left panel" : "Open left panel"}>
          <Menu size={20} />
        </button>
  ...
  const handleShare = () => {
    ...
    const providerLabel = getProviderProfileLabel(providerProfiles.find((profile) => profile.id === selectedProviderId) || providerProfiles.find((profile) => profile.providerType === selectedProviderId));
    const exportText = `AI Studio Conversation: ${activeChat.title}\nProvider: ${providerLabel}\nModel: ${getModelLabel(activeChat.model, providerProfiles)}\n\n` + (activeChat.messages || []).map((m) => `[${m.sender.toUpperCase()}]: ${m.text}`).join("\n\n");
  ...
  return (
    <div className="h-16 border-b border-[#E7E7E7] dark:border-[#23272A] bg-white dark:bg-[#16191B] flex items-center justify-between px-4 md:px-6 flex-shrink-0 z-30 select-none transition-colors duration-200">
      <div className="flex items-center gap-3 min-w-0">
        <button onClick={() => dispatch(toggleSidebar())} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-[#23272A] text-[#737373] dark:text-[#94A3B8] cursor-pointer transition-colors" title={sidebarOpen ? "Hide left panel" : "Open left panel"}>
          <Menu size={20} />
        </button>
        <div className="flex items-center gap-2 min-w-0">
          {isEditingTitle ? (
            <input type="text" value={titleInput} onChange={(e) => setTitleInput(e.target.value)} onBlur={handleRenameSubmit} onKeyDown={(e) => { if (e.key === "Enter") handleRenameSubmit(); if (e.key === "Escape") setIsEditingTitle(false); }} className="text-base font-semibold border-b border-[#245955] dark:border-[#347d78] text-[#171717] dark:text-[#ECEFF1] bg-transparent focus:outline-none px-0.5" autoFocus />
          ) : (
            <>
              <h2 className="text-base font-semibold text-[#171717] dark:text-[#ECEFF1] truncate font-montserrat transition-colors">{activeChat.title}</h2>
              <button onClick={() => { setTitleInput(activeChat.title); setIsEditingTitle(true); }} className="p-1 rounded text-[#737373] dark:text-[#94A3B8] hover:text-[#171717] dark:hover:text-[#ECEFF1] hover:bg-slate-100 dark:hover:bg-[#23272A] transition-colors" title="Edit Title"><Edit3 size={14} /></button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
        <div className="relative hidden sm:block" ref={modelRef}>
          <button onClick={() => dispatch(toggleModelDropdown())} className="h-10 px-3.5 border border-[#E7E7E7] dark:border-[#23272A] hover:border-[#cbd5e1] dark:hover:border-zinc-700 rounded-lg bg-white dark:bg-[#16191B] flex items-center gap-2 text-xs font-semibold text-[#171717] dark:text-[#ECEFF1] shadow-sm transition-colors cursor-pointer">
            <span>{getModelLabel(currentModel, providerProfiles)}</span>
          ...
          {modelDropdownOpen && (
            <div className="absolute right-0 mt-1.5 w-72 max-h-80 overflow-y-auto bg-white dark:bg-[#1E2326] border border-[#E7E7E7] dark:border-[#23272A] rounded-lg shadow-lg py-1.5 z-50 transition-colors">
              {providerModels.map((model) => (
                ...
                >
                  <span className={currentModel === model.id ? "font-semibold text-[#245955] dark:text-[#347d78]" : "text-[#737373] dark:text-[#94A3B8]"}>{model.label}</span>
                  {currentModel === model.id && <Check size={14} className="text-[#245955] dark:text-[#347d78]" />}
                </button>
              ))}
              <div className="border-t border-[#E7E7E7] dark:border-[#23272A] mt-1.5 pt-1.5 px-4 pb-0.5">
                <div className="text-[10px] text-[#737373] dark:text-[#94A3B8] leading-normal">Only models supported by the selected provider are shown here.</div>
              </div>
            </div>
          )}
        </div>
        ...
        <button onClick={() => dispatch(toggleTheme())} className="p-2.5 border border-[#E7E7E7] dark:border-[#23272A] bg-white dark:bg-[#16191B] hover:bg-slate-50 dark:hover:bg-[#23272A] text-[#737373] dark:text-[#94A3B8] hover:text-[#171717] dark:hover:text-[#ECEFF1] rounded-lg shadow-sm cursor-pointer transition-colors" title={theme === "light" ? "Switch to Dark Mode" : "Switch to Light Mode"}>
          {theme === "light" ? <Moon size={16} /> : <Sun size={16} />}
        </button>

        <button onClick={handleShare} className="relative p-2.5 border border-[#E7E7E7] dark:border-[#23272A] bg-white dark:bg-[#16191B] hover:bg-slate-50 dark:hover:bg-[#23272A] text-[#737373] dark:text-[#94A3B8] hover:text-[#171717] dark:hover:text-[#ECEFF1] rounded-lg shadow-sm cursor-pointer transition-colors" title="Share / Copy Conversation">
          {copiedToast ? <Check size={16} className="text-emerald-500" /> : <Share2 size={16} />}
          {copiedToast && <span className="absolute right-0 top-12 px-2.5 py-1 bg-emerald-600 text-white text-[10px] font-semibold rounded shadow-lg whitespace-nowrap z-50">Copied to clipboard!</span>}
        </button>
      </div>
    </div>
  );
